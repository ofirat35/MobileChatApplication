...
namespace ChatApp.Core.Application.Features.Queries.Swipes
{
    public class GetUsersToSwipeQueryHandler(ISwiperService swiperService)
      : BaseCommandHandler, IRequestHandler<GetUsersToSwipeRequestQuery, ResponseModel<List<UserProfile>>>
    {
        public async Task<ResponseModel<List<UserProfile>>> Handle(GetUsersToSwipeRequestQuery request, CancellationToken cancellationToken)
        {
    ...
    public class GetUsersToSwipeRequestQuery : IRequest<ResponseModel<List<UserProfile>>>
    {
